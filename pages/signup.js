      ...
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-800 rounded-lg shadow-md text-center">
        ...
        <form onSubmit={handleSignup} aria-label="Signup Form" className="space-y-4">
          ...
          <div>
            ...
            <input id="password" type="password" value={password} onChange={(e) => setEmail(e.target.value)} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-[#2046f5] focus:border-transparent" required aria-required="true" />
          </div>
          <button type="submit" disabled={loading} className="w-full p-2 bg-[#2046f5] text-white rounded-md hover:bg-[#1a3cd1] focus:outline-none focus:ring-2 focus:ring-[#2046f5] focus:ring-opacity-50 transition-all duration-200" aria-label="Signup Button">
            {loading ? 'Signing up...' : 'Signup'}
          </button>
        </form>
        <p className="text-center mt-4 text-sm text-gray-600 dark:text-gray-400">Already have an account? <Link href="/login" className="text-[#2046f5] hover:underline">Login</Link></p>
      </div>
    </div>
  );
}
